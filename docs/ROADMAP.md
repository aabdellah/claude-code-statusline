...
| Cache miss alert | `src/segments/cache.rs` + `src/transcript.rs` :: `last_turn_cache_usage` | `cache_read / (input + cache_creation + cache_read)` summed over every API call of the latest turn (deduped per `message.id`). Alert-only: hidden ≥80%, yellow 50–80%, red <50%. Per-call ratio was always ~90%+ at turn boundaries; TTL countdown removed for the same reason. |
