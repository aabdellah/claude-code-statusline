...
//!
//! Layered fallback, cheapest reliable source first. Unix:
...
//! any) → $COLUMNS (CC exports it before running us) → None.
//!
//! The override / $COLUMNS / debug-dump steps are shared; each OS supplies
//! `probe_tty` (everything that talks to a live terminal) and
//! `read_cache_files` (the shell-hook caches — Unix only).
...
pub fn detect_term_width(cfg: &Config) -> Option<u16> {
    ...
    if let Some(override_val) = cfg.width_override {
        ...
        // 1. Anything that can ask a live terminal directly.
        result = imp::probe_tty(&mut trace);

        // 2. $COLUMNS env var. CC exports it before running us; interactive
        //    shells rarely do.
        if result.is_none() {
        ...
        // 3. Shell-hook cache files (legacy fallback).
        if result.is_none() {
            result = imp::read_cache_files(&mut trace);
        }
    }
    ...
    if cfg.debug_width {
        ...
        if let Ok(json) = serde_json::to_string_pretty(&payload) {
            let _ = fs::write(platform::shared_tmp_dir().join("cc-statusline-width.json"), json);
        }
    }
...
mod imp {
    ...
    pub fn probe_tty(trace: &mut Trace) -> Option<u16> {
        ...
        if let Some(c) = ancestor_walk(trace) {
            return Some(c);
        }
    ...
    pub fn read_cache_files(trace: &mut Trace) -> Option<u16> {
        // Per-PTY cache file (shell hook writes /tmp/cc-term-width-<tty>).
        if let Some(c) = read_per_pty_cache(trace) {
            return Some(c);
        }
...
mod imp {
    ...
    pub fn probe_tty(trace: &mut Trace) -> Option<u16> {
        // 1. Standard streams — CC pipes these, so they're rarely consoles.
        if let Some(c) = platform::console_columns(std::io::stdout().as_raw_handle()) {
            trace.insert("stdout", c.to_string());
            return Some(c);
        } else if let Some(c) = platform::console_columns(std::io::stderr().as_raw_handle()) {
            trace.insert("stderr", c.to_string());
            return Some(c);
        }
        trace.insert("stdout", "no-console".into());
        trace.insert("stderr", "no-console".into());

        // 2. CONOUT$ — the console this process is attached to, when CC
        //    spawned us with one. Fails cleanly when detached.
        match OpenOptions::new().read(true).write(true).open("CONOUT$") {
            Ok(con) => {
                if let Some(c) = platform::console_columns(con.as_raw_handle()) {
                    trace.insert("conout", c.to_string());
                    return Some(c);
                }
                trace.insert("conout", "no-cols".into());
            }
            Err(_) => {
                trace.insert("conout", "open-failed".into());
            }
        }
        None
    }
    ...
    pub fn read_cache_files(trace: &mut Trace) -> Option<u16> {
        // The /tmp/cc-term-width* files come from a Unix shell hook; there
        // is no Windows equivalent.
        trace.insert("tty_cache", "n/a".into());
        None
    }
}
